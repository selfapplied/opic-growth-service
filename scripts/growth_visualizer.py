...
def load_all_snapshots(growth_dir=None):
    ...
    if growth_dir is None:
        ...
        if not growth_dir.is_absolute():
            growth_dir = REPO_DIR / growth_dir
    
    snapshots = []
    for snapshot_file in sorted(growth_dir.glob('*.yaml')):
        try:
...
def generate_timeline_svg(snapshots, width=800, height=400):
    ...
    max_count = max(s['count'] for s in snapshots) or 1
    padding = 40
...
def main():
    ...
    if output_type == 'rings':
        ...
        output_file = diagrams_dir / 'growth-rings.svg'
    else:
        svg = generate_timeline_svg(snapshots)
        output_file = diagrams_dir / 'growth-timeline.svg'
